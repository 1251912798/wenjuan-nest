...
export class StatService {
    ...
    private _genAnswersInfo(question, answerList = []) {
        ...
        answerList.forEach((a) => {
            ...
            if (type === 'questionRadio' || type === 'questionSelectBox') {
                // 单选
                ...
                // 多选
                res[componentFeId] = (value as any[]).map((v) =>
                    this._getCheckboxOptText(v, props),
                );
            } else {
                // 其他
    ...
    async getQuestionStatListAndCount(
        ...
        const total = await this.answerService.total(questionId);
    ...
    async getComponentStat(questionId: string, componentFeId: string) {
        ...
        answers.forEach((a) => {
            const { answerList = [] } = a;
            answerList.forEach((a) => {
                const { componentId: c, value } = a;
                if (c !== componentFeId) return;
